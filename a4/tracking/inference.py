...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if (noisyDistance == None):
            ...
            for p in self.particles:
                ...
                if (noisyDistance is not None):
                    trueDistance = util.manhattanDistance(p, pacmanPosition)
                    pro = emissionModel[trueDistance]
                allPossible[p] += pro
        ...
        allPossible.normalize()
        new_particles = util.nSample(allPossible.values(), allPossible.keys(), self.numParticles)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        new_legal_position = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        while len(self.particles) < self.numParticles:
            for p in new_legal_position:
                if len(self.particles) < self.numParticles:
                    self.particles.append(p)
        "*** END YOUR CODE HERE ***"
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                if (noisyDistances[i] is not None):
                    ...
                    pro *= emissionModels[i][trueDistance]
            allPossible[p] += pro
        ...
        for i in range(self.numGhosts):
            if (noisyDistances[i] == None):
                for j in range(len(new_particles)):
                    new_particles[j] = self.getParticleWithGhostInJail(new_particles[j], i);
